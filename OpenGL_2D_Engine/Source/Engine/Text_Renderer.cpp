...
{
    this->World_Origin = glm::vec2(width / 2, height / 2);
...
void TextRenderer::Load(std::string font, unsigned int fontSize)
{
    ...
    // load font as face
    FT_Face face;
    if (FT_New_Face(ft, (FONT_PATH + font).c_str(), 0, &face))
...
void TextRenderer::RenderText(std::string text, float x, float y, float scale, glm::vec3 color)
{
    ...
    for (c = text.begin(); c != text.end(); c++)
    {
        ...
        float xpos = World_Origin.x + x + ch.Bearing.x * scale;
        float ypos = World_Origin.y - y + (this->Characters['H'].Bearing.y - ch.Bearing.y) * scale;
